...
export default function InviteSignupClient({ inviteToken, lockedEmail }: Props) {
  ...
  const onSubmit = async (data: FormData) => {
    setSubmitting(true);
    try {
      // If there's an existing session, sign out first to avoid session conflicts
      if (existingRole) {
          ...
          <div className="mb-4 flex gap-3 items-start bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
            ...
            <div>
              ...
              <p className="text-xs text-amber-600 mt-0.5">
                ...
                will sign you out and create your new coach account.
              </p>
            </div>
          </div>
        )}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input
                    ...
                    className="border-[#d8e0d8] bg-white text-[#0f1f10] placeholder-[#a0b0a0] focus-visible:ring-[#3a7d44]"
                  />
                  {errors.firstName && (
